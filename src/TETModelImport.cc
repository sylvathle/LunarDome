...
void TETModelImport::DataRead(G4String eleFile, G4String nodeFile)
{
  ...
  //G4cout << "_________________________________________________________________________________" << numEle << G4endl;
  //numEle = (int) numEle/100;
  ...
  //for(G4int i=0; i<2; ++i)
  {
    ...
    ifpEle >> mat;
    //if (mat != 8700 && mat!=1000) {continue;}

    fEleVector.push_back(ele);

    //G4cout << fMaterialVector.size() << G4endl;
    ...
    //G4cout << i << " " << mat << " " << ele[0] << " " << ele[1] << " " << ele[2] << " " << ele[3] << " "  << G4endl;
  }
  ifpEle.close();
}
