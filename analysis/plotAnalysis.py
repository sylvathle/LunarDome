...
res_dir = "../results_backup/ICRP145/"
...
df_bon2020 = pd.read_csv("../scripts/gcr_2020-02-01_2021-02-01.csv")
...
for it, thick in enumerate(list_thick):
  ...
  for opart in listopart:
    ...
    else: ax[it].plot(dfo['okE_c']/4,dfo['count']*4,'.',markersize=4,label="alpha",color=Ioncolors[opart])
    #print (dfo["count"])
...
ax[0].set_ylabel("Flux $[m^{2} \cdot sr \cdot s \cdot MeV]^{-1}{}^*$",fontsize=18)
#ax.grid(True)
...
for it, thick in enumerate(list_thick):
  ...
  ax[it].grid(True)
  #ax[it].set_ylim(ymin=1e-10,ymax=10)

ax[0].legend(fontsize=13, ncol=2)
ax[0].set_ylabel("Flux $[m^{2}\cdot sr \cdot s \cdot MeV/n]^{-1}$",fontsize=18)
#ax.grid(True)
#plt.set_ylabel("Particle Flux (N particle per year)")
#plt.set_xlabel("Particle Energy (MeV)")
fig.text(0.5, 0.03, 'Particle Energy (MeV)', ha='center',fontsize=18)
